...
export class ExtendedUserComponent implements OnInit {
  ...
  extendedUsers = signal<IExtendedUser[]>([]);
  isLoading = false;

  sortState = sortStateSignal({});

  public readonly router = inject(Router);
  protected readonly extendedUserService = inject(ExtendedUserService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IExtendedUser): number => this.extendedUserService.getExtendedUserIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (this.extendedUsers().length === 0) {
            this.load();
          } else {
            this.extendedUsers.set(this.refineData(this.extendedUsers()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.extendedUsers.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IExtendedUser[]): IExtendedUser[] {
